...
public class CustomerBOImpl implements CustomerBO {
        ...
        try {
            ...
            if (!b1){
                ...
                for (OrderDTO orderDTO : customer.getOrders()){
                    boolean b2 = orderDAO.add(new Order(orderDTO.getOrderId(), orderDTO.getDateTime(), orderDTO.getTotal(), orderDTO.getCusId(), orderDTO.getUserId(), orderDTO.getPaymentMethod(), orderDTO.getOrderType()));
                    ...
                    if (!b2) {
                        ...
                        for (OrderDetailDTO orderDetailDTO : customer.getDetails()){
                            boolean b3 = orderDetailsDAO.add(new OrderDetail(orderDetailDTO.getQty(), orderDetailDTO.getUnitPrice(), orderDetailDTO.getDisCount(), orderDetailDTO.getOrderId(), orderDetailDTO.getProductId(),orderDetailDTO.getProductName(),orderDetailDTO.getRefId(),orderDetailDTO.getAmount()));
    ...
    @Override
    public boolean ifCustomerIdExist(String id) throws SQLException, ClassNotFoundException {
        return customerDAO.ifCustomerIdExist(id);
    }

    public boolean ifCustomerTpExist(int tp) throws SQLException, ClassNotFoundException {
        return customerDAO.ifCustomerTpExist(tp);
    }

    @Override
    public CustomerDTO searchCustomerTp(int tp) throws SQLException, ClassNotFoundException {
        Customer search = customerDAO.searchCustomerTp(tp);
        return new CustomerDTO(search.getCusId(),search.getCusType(),search.getCusName(),search.getCusAddress(),search.getCusCity(),search.getCusProvince(),search.getCusContact());
    }
