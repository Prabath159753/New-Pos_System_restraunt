...
public class SystemReportBOImpl implements SystemReportBO {
    ...
    public ArrayList<OrderDTO> getAllOrders() throws Exception {
        ...
        for (Order order:all){
            orderDTOS.add(new OrderDTO(order.getOrderId(),order.getDateTime(),order.getTotal(),order.getCusId(),order.getUserId(),order.getPaymentMethod(),order.getOrderType()));
        }
        return orderDTOS;
    }
    ...
    public OrderDTO getOrder(String id) throws Exception {
        ...
        return new OrderDTO(order.getOrderId(),order.getDateTime(),order.getTotal(),order.getCusId(),order.getUserId(),order.getPaymentMethod(),order.getOrderType());
    }
    ...
    public ArrayList<OrderDetailDTO> getAllOrderDetails() throws Exception {
        ...
        for (OrderDetail orderDetail: all){
            orderDetailDTOS.add(new OrderDetailDTO(orderDetail.getQty(),orderDetail.getUnitPrice(),orderDetail.getDisCount(),orderDetail.getOrderId(),orderDetail.getProductId(),orderDetail.getProductName(),orderDetail.getRefId(),orderDetail.getAmount()));
        }
    ...
    @Override
    public ArrayList<OrderDTO> getAllOrdersByPaymentMethod(String paymentMethod) throws SQLException, ClassNotFoundException {
        List<Order> allOrdersByPaymentMethod = orderDAO.getAllOrdersByPaymentMethod(paymentMethod);
        ArrayList<OrderDTO> orderDTOS = new ArrayList<>();
        for (Order order:allOrdersByPaymentMethod){
            orderDTOS.add(new OrderDTO(order.getOrderId(),order.getDateTime(),order.getTotal(),order.getCusId(),order.getUserId(),order.getPaymentMethod(),order.getOrderType() ));
        }
        return orderDTOS;
    }
