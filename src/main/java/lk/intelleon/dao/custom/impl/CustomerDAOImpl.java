...
public class CustomerDAOImpl implements CustomerDAO {
    ...
    @Override
    public boolean ifCustomerIdExist(String custId) throws SQLException, ClassNotFoundException {
        return CrudUtil.executeQuery("SELECT id FROM customer WHERE id=?", custId).next();
    }

    @Override
    public boolean ifCustomerTpExist(int tp) throws SQLException, ClassNotFoundException {
        return CrudUtil.executeQuery("SELECT id FROM customer WHERE contact=?", tp).next();
    }

    @Override
    public Customer searchCustomerTp(int tp) throws SQLException, ClassNotFoundException {
        ResultSet resultSet = CrudUtil.execute("SELECT * FROM Customer WHERE contact=?", tp);
